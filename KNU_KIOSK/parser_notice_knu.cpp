...
int add_title_knu(GumboNode* tbody, const wchar_t* category) {
...
int find_tbody_knu(GumboNode* node, const wchar_t* category) {
...
int parser_notice_knu_1() {
	...
	find_tbody_knu(parse->root, L"공지");
...
int parser_notice_knu_2() {
	...
	GumboOutput* parse = gumbo_parse(html.c_str());
	find_tbody_knu(parse->root, L"학사/장학");
	gumbo_destroy_output(&kGumboDefaultOptions, parse);
...
int parser_notice_knu_3() {
	...
	GumboOutput* parse = gumbo_parse(html.c_str());
	find_tbody_knu(parse->root, L"모집/안내/공고");
	gumbo_destroy_output(&kGumboDefaultOptions, parse);
...
int parser_notice_knu_4() {
	...
	GumboOutput* parse = gumbo_parse(html.c_str());
	find_tbody_knu(parse->root, L"행사/뉴스");
	gumbo_destroy_output(&kGumboDefaultOptions, parse);
...
int parser_notice_knu_5() {
	...
	GumboOutput* parse = gumbo_parse(html.c_str());
	find_tbody_knu(parse->root, L"교육/연구");
	gumbo_destroy_output(&kGumboDefaultOptions, parse);
...
int parser_notice_dormi() {
	...
	GumboOutput* parse = gumbo_parse(html.c_str());
	find_tbody_knu(parse->root, L"기숙사");
	gumbo_destroy_output(&kGumboDefaultOptions, parse);
